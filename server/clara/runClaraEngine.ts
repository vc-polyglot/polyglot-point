...
export type RunClaraEngineInput = {
  ...
  engineVariant?: EngineVariant; // overrides env CLARA_ENGINE
};

export type RunClaraEngineDeps = Readonly<{
  openaiClient?: OpenAI; // inyección opcional (tests)
}>;
...
function validateClaraRaw(raw: string): { ok: boolean; cleaned: string } {
  ...
  // Si por error el modelo devuelve JSON "de otro contrato", lo rechazamos.
...
// SOLUCIÓN 1: deps separado (tests sin red, producción intacta)
export async function runClaraEngine(
  ...
  const history = toHistoryMsgArray(input.history).slice(-6);
  ...
  const temperature = getEnvFloat("CLARA_TEMPERATURE", 0.4);
  const maxTokens = getEnvInt("CLARA_MAX_TOKENS", 550);
