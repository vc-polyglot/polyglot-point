...
import { conversationSettingsSchema } from "../shared/schema";
  ...
  app.post("/api/language/change", async (req, res) => {
    try {
      ...
      if (!language || !sessionId) {
        console.error(`âŒ MISSING PARAMETERS: language=${language}, sessionId=${sessionId}`);
      ...
      if (result.success) {
        ...
        console.error(`âŒ LANGUAGE CHANGE FAILED: ${result.errors.join(', ')}`);
